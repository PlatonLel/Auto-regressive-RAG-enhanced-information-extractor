...
class IeGenerator(BaseJointRE):
    ...
    def compute_token_embeddings(self, x):
        ...
        lengths = x['seq_length'].cpu()  # pack_padded_sequence requires lengths on CPU
        packed_embed = pack_padded_sequence(embed, lengths, batch_first=True, enforce_sorted=False)
        
        # Apply LSTM
        packed_output, _ = self.rnn(packed_embed)
        
        # Unpack sequence
        embed, _ = pad_packed_sequence(packed_output, batch_first=True)

...
def train(model, optimizer, train_data, eval_data,
    ...
    for step in pbar:
        ...
        try:
            ...
            logging.error(f"Error in step {step + 1}: {type(e).__name__} - {e}", exc_info=True)
            continue
        ...
        if (int(step + 1)) % save_interval == 0:
            ...
            logging.info(f"Current F1: {current_f1}")
